...
__device__ int index_to_position(const int *index, const int *strides,
     ...
     * position in storage based on strides. Args: index: index tuple of ints
     *    strides: tensor strides
     *    num_dims: number of dimensions in the tensor, e.g. shape/strides of
     * [2, 3, 4] has 3 dimensions
     *
     * Returns:
...
__device__ void to_index(int ordinal, const int *shape, int *out_index,
                         int num_dims) {
    /**
     * Convert an ordinal to an index in the shape. Should ensure that
     * enumerating position 0 ... size of a tensor produces every index exactly
     * once. It may not be the inverse of index_to_position. Args: ordinal:
     * ordinal position to convert shape: tensor shape out_index: return index
     * corresponding to position num_dims: number of dimensions in the tensor
     *
     * Returns:
...
void MatrixMultiply(float *out, int *out_shape, int *out_strides,
    ...
    cudaMemcpy(out, d_out, batch * m * p * sizeof(float),
...
void tensorMap(float *out, int *out_shape, int *out_strides, int out_size,
    ...
    cudaMemcpy(d_in, in_storage, in_size * sizeof(float),
               cudaMemcpyHostToDevice);
...
void tensorReduce(float *out, int *out_shape, int *out_strides, int out_size,
    ...
    reduceKernel<<<blocksPerGrid, threadsPerBlock>>>(
        d_out, d_out_shape, d_out_strides, out_size, d_a, d_a_shape,
        d_a_strides, reduce_dim, reduce_value, shape_size, fn_id);
